...
l'utilisateur et sa machine plut\^ot qu'en en utilisant deux.
...
Bien que similaire, la base de donn\'ees est l\'eg\`erement diff\'erente
sur le serveur par rapport au client. Les changements prennent en compte
qu'il peut exister plusieurs machines pour un utilisateur mais aussi que
plusieurs utilisateurs existent au sein d'une compagnie.
\begin{lstlisting}[language=json]
User :
{ "_id",
  "id_admin",
...
Comme nous l'avons vu pr\'ec\'edemment, \textit{MongoDB} avec \textit{GridFS}
permet de stocker des fichiers. N\'eanmoins nous avons d\'ecid\'e d'utiliser,
au moins dans un premier temps, le syst\`eme de fichier disponible sur le
serveur : celui-ci ayant de meilleurs performances en lecture et \'ecriture.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=17cm]{softwareDesign/fileSystemServer.png}
