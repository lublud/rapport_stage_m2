...
pr\'esente sur le serveur, celle du client permettrait de ne stocker que les
...
principales diff\'erences sont expos\'ees table~\ref{tabSQLNoSQL}.
...
Nous avons dans un premier temps penser au mod\`ele relationnel avec une base
...
La signification des diff\'erents champs pr\'esents figure~\ref{dbRelClient} se
trouve en annexe~\ref{annBDR}.
...
soit dans l'imm\'ediat ou dans une futur version de l'application.

Donc bien que dans un premier temps nous ayons d\'ecid\'e d'utiliser le
syst\`eme de fichier en place sur le serveur, nous avons pris cette
fonctionnalit\'e en compte.
...
la collection \textit{User}. Ce choix s'est fait puisque, sur la machine
client, il n'y a que la machine sur laquelle le logiciel est install\'e qui
est pr\'esente en base de donn\'ees. De plus, l'acc\`es aux donn\'ees eest
plus simple et plus rapide en utilisant une collection regroupant l'utilisateur
et sa machine plut\^ot qu'en en utilisant deux.
...
\begin{table}[h!]
  ...
  \begin{tabular}{lm{6cm}m{6cm}}
   \rowcolor{arkred} 
    \arrayrulecolor{gray73}\hline
    \color{white} \textbf{Champ} & \color{white} \textbf{Information}\\
    \hline
  ...
  \end{tabular}
  \caption{\label{tabDBRUser} Information champ --- User}
\end{table}
...
\begin{table}[h!]
  ...
  \end{tabular}
  \caption{\label{tabDBRMachine} Information champ --- Machine}
\end{table}
...
\begin{table}[h!]
  ...
  \end{tabular}
  \caption{\label{tabDBRMachine} Information champ --- File}
\end{table}
...
\begin{table}[h!]
  ...
  \end{tabular}
  \caption{\label{tabDBRMachine} Information champ --- Chunk}
\end{table}
\end{savenotes}
